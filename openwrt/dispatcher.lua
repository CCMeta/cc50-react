...
function dispatch(request)
	...
	if type(auth) == "table" and type(auth.methods) == "table" and #auth.methods > 0 then
		...
		if not (sid and sdat and sacl) and auth.login then
			...
			http.header("Set-Cookie", 'sysauth=%s; path=%s; SameSite=None; %s' %{
				sid, "/", http.getenv("HTTPS") == "on" and "secure" or ""
			})
			http.header("Access-Control-Allow-Origin", http.getenv("HTTP_ORIGIN") or "*")
	...
	if http.getenv("REQUEST_METHOD") == "OPTIONS" then
		...
		luci.http.header("Access-Control-Allow-Credentials", "true")
